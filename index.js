...
function validate(){
    ...
    var validNumber = validateInputs("number", "nerror", "Phone Number", 10, numberRegex) ;
    if(document.getElementById("number").value.trim().length != 10){
        number = false;
        nerror.innerHTML = "*  Phone Number should contains exactly 10 characters.<br><br>";
    }
    var validMarks = validateMarks("sub", "serror", "Marks");
    var validEmail = validInputs("lname", "lerror", "Last Name", 30, emailRegex)
    
    return fnameValid && lnameValid && validNumber && validMarks && validEmail;
